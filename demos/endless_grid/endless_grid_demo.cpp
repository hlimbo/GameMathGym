...
const char* WINDOW_NAME = "Endless Grid Demo";
SDL_Window* win = NULL;
...
GLuint endlessGridShaderProgramId;
const std::string endlessGridVertShaderSrc("shaders/endless_grid.vert");
const std::string endlessGridFragShaderSrc("shaders/endless_grid.frag");
GLuint endlessGridVAO;
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char **argv) {
  ...
  /* Shader Initialization */
  {
    ...
    vertShaderId = ShaderUtils::LoadAndCreateShaderSource(endlessGridVertShaderSrc, GL_VERTEX_SHADER);
    fragShaderId = ShaderUtils::LoadAndCreateShaderSource(endlessGridFragShaderSrc, GL_FRAGMENT_SHADER);

    ShaderUtils::VerifyShaderCompilationStatus(vertShaderId, endlessGridVertShaderSrc);
    ShaderUtils::VerifyShaderCompilationStatus(fragShaderId, endlessGridFragShaderSrc);

    endlessGridShaderProgramId = ShaderUtils::CreateShaderProgram(vertShaderId, fragShaderId);
    ShaderUtils::VerifyShaderProgramLinkStatus(endlessGridShaderProgramId);
  ...
  /* Create Endless Grid VAO */
  {
    glGenVertexArrays(1, &endlessGridVAO);
  }
